...
function ExpenseCategories() {
  ...
  useEffect(() => {
    const fetchHistoricalData = async () => {
      ...
      const q = query(
        collection(db, "temperature_snapshots"),
  ...
  useEffect(() => {
    if (realtimeData) {
      ...
      const newDataPoint = {
        x: now,
        y: realtimeData.dht?.temperature,
      };
  ...
  useEffect(() => {
    const interval = setInterval(async () => {
      if (snapshotRef.current.length > 0) {
        try {
          await addDoc(collection(db, "temperature_snapshots"), {
  ...
  const chartOptions = {
    ...
    plugins: {
      ...
      title: {
        display: true,
        text: `Temperature (Last ${timeRange} Minutes)`,
      },
    },
    scales: {
      x: {
      ...
      y: {
        title: { display: true, text: "Temperature Value" },
      },
    },
  };
  ...
  return (
    <Container>
      <h1 className="text-center m-4">Temperature Live Chart</h1>
